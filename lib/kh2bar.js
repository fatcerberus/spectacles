/**
 *  kh2Bar for Spherical engines
 *  a segmented health gauge inspired by Kingdom Hearts
...
function HPGauge(capacity, sectorSize, color, maxSectors)
{
	if (arguments.length < 1) {
		throw new RangeError("HPGauge requires 1+ parameters", -1);
	}

	sectorSize = sectorSize !== undefined ? sectorSize : 100;
	color = color !== undefined ? color.clone() : new Color(0, 255, 0, 255);
...
HPGauge.prototype.draw = function(x, y, width, height)
{
	if (arguments.length < 4) {
		throw new RangeError("HPGauge#draw requires 4+ parameters");
	}
	...
	if (barInUse < this.sectorSize && numReservesFilled > 0) {
		OutlinedRectangle(x, y, width, barHeight, Color.mix(borderColor, new Color(0, 0, 0, 0), 25, 75));
		drawSegment(x + 1, y + 1, width - 2, barHeight - 2, Color.mix(fillColor, new Color(0, 0, 0, 0), 25, 75));
	}
...
function drawSegment(x, y, width, height, color)
{
	...
	var dimColor = Color.mix(color, new Color(0, 0, 0, color.alpha), 66, 33);
