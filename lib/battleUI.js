...
class HPGauge
{
	...
	{
		this.borderColor = Color.Black.fade(color.alpha);
		this.capacity = capacity;
		this.colorFadeDuration = 0;
		this.colorFadeTimer = 0;
		this.damage = 0;
		this.damageColor = new Color(0.75, 0.0, 0.0, color.alpha);
		this.damageFadeness = 1.0;
		this.drainSpeed = 2.0;
		this.emptyColor = new Color(0.125, 0.125, 0.125, color.alpha);
		this.fadeSpeed = 0.0;
		this.fadeness = 1.0;
...
class MPGauge
{
	...
	draw(x, y, size)
	{
		...
		if (this.capacity > 0) {
			...
			var outerFillColor = Color.mix(this.color, Color.Black.fade(this.color.alpha));
			var outerUsageColor = this.usageColor;
			var innerUsageColor = Color.mix(this.usageColor, Color.Black.fade(this.usageColor.alpha));
...
function drawSegment(x, y, width, height, color)
{
	...
	let dimColor = Color.mix(color, Color.Black.fade(color.alpha), 66, 33);
...
function drawText(font, x, y, shadowDistance, color, text, alignment = 'left')
{
	...
	font.drawText(screen, x + shadowDistance, y + shadowDistance, text, Color.Black.fade(color.alpha));
