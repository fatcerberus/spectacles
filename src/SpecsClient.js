...
function SpecsClient(hostname, person)
{
	...
	this.person = person;
	this.isReady = false;
	term.print("Connecting to person server at " + hostname + ":812");
	threads.create(this);
}
	...
	if (pendingBytes > 0) {
		...
		for (var i = 0; i < lines.length; ++i) {
			if (lines[i] == 'specs client?') {
				...
				term.print("Successfully connected to person server");
				term.print("Now controlling '" + this.person + "' remotely");
				this.isReady = true;
