...
function SpecsServer()
{
	...
	if (this.socket != null) {
		...
		mini.Console.writeLine("Specs server started running on port 812");
	} else {
		mini.Console.writeLine("Couldn't start Specs server");
	}
}
...
SpecsServer.prototype.update = function()
{
	...
	if (socket != null) {
		mini.Console.writeLine("Accepted connection from " + socket.getRemoteAddress() + ":" + socket.getRemotePort());
	...
	for (var i = 0; i < this.connections.length; ++i) {
		if (!this.connections[i].isConnected()) {
			...
			mini.Console.writeLine(ip + ":" + port + " has disconnected");
...
SpecsServerSocket.prototype.update = function()
{
	...
	for (var i = 0; i < lines.length; ++i) {
		if (lines[i] == 'specs client.' && !this.isSpecsClient) {
			mini.Console.writeLine(this.ip + ":" + this.port + " is a Specs Engine client");
			this.isSpecsClient = true;
		}
		if (!this.isSpecsClient) continue;
		if (lines[i].substr(0, 10) == 'person-id ') {
			this.person = lines[i].substr(10);
			mini.Console.writeLine(this.ip + ":" + this.port + " took control of '" + this.person + "'");
