...
function AIContext(unit, battle, aiType)
{
	...
	mini.Console.write("Initializing AI context for " + unit.fullName);
	this.battle = battle;
	this.data = {};
...
{
	mini.Console.write("Shutting down AI for " + this.unit.fullName);
...
AIContext.prototype.checkPhase = function(allowEvents)
{
	...
	if (allowEvents && this.phase > lastPhase) {
		mini.Console.write(this.unit.name + " is entering Phase " + this.phase);
...
AIContext.prototype.getNextMove = function()
{
	...
	do {
		if (this.moveQueue.length == 0) {
			mini.Console.write("Deferring to AI for " + this.unit.name + "'s next move");
			...
			if (this.moveQueue.length == 0) {
				mini.Console.write("No moves queued for " + this.unit.name + ", using default");
		...
		do {
			...
			if (!isMoveUsable) {
				mini.Console.write("Discarding " + this.unit.name + "'s " + candidateMove.usable.name + ", not usable");
			}
...
AIContext.prototype.predictItemTurns = function(itemID)
{
	...
	mini.Console.write(this.unit.name + " considering " + Game.items[itemID].name);
...
AIContext.prototype.predictSkillTurns = function(skillID)
{
	...
	mini.Console.write(this.unit.name + " considering " + Game.skills[skillID].name);
	mini.Console.append("next: " + forecast[0].unit.name);
	return forecast;
};
...
AIContext.prototype.queueItem = function(itemID, unitID)
{
	...
	mini.Console.write(this.unit.name + " queued use of item " + itemToUse.name);
};
...
AIContext.prototype.queueSkill = function(skillID, unitID, predicate)
{
	...
	mini.Console.write(this.unit.name + " queued use of skill " + skillToUse.name);
};
...
AIContext.prototype.queueWeapon = function(weaponID)
{
	...
	mini.Console.write(this.unit.name + " queued weapon change to " + weaponDef.name);
};
...
AIContext.prototype.setDefaultSkill = function(skillID)
{
	...
	mini.Console.write(this.unit.name + "'s default skill set to " + Game.skills[skillID].name);
};
