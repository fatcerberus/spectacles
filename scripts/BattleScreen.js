...
function BattleScreen()
{
	this.$startThread = function()
	{
		...
		this.$hud.show();
		new Scenario()
			.tween(this, 0.5, 'easeOutExpo', { $hudFadeness: 1.0 })
			.run();
	};
	
	this.$actorTypes = {
		enemy: { shownInHUD: false, isMirrored: false },
		party: { shownInHUD: true, isMirrored: true, }
	};
	this.$actors = {};
	...
	this.createActor = function(name, position, row, alignment, alreadyThere)
	{
		...
		if (this.$actorTypes[alignment].shownInHUD) {
			this.$hud.switchOut(position, name, 0, 0);
		}
		return sprite;
	};
	...
	this.createEnemyHPGauge = function(name, capacity)
	{
	...
	this.highlightActor = function(name)
	{
